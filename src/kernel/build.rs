...
fn visit_dirs(directory: &Path, callback: &mut dyn FnMut(&DirEntry)) {
    if directory.is_dir() {
...
fn name_from_path<'a>(path: &'a Path) -> Result<&'a str> {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    visit_dirs(Path::new("src"), &mut |entry| {
        ...
        match path.extension() {
            Some(extension) if ext.eq(&OsString::from("real")) => {
                let file_name = name_from_path(path)?;
                // TODO: Finish this
            },

            Some(extension) if ext.eq(&OsString::from("inc")) => {
